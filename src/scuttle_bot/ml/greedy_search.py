...
def cross_val_accuracy(model_factory: Callable, X, y, random_states, plot: bool = False) -> float:
    """Mean test accuracy of ``model_factory(random_state)`` across several
    train/test splits (one per random_state) -- the same way the trainers
    report a config's accuracy. Plotting/saving stay off by default because
    this is called once per candidate config inside the search loop."""
    accuracies = []
...
def greedy_hyperparameter_search(
    ...
    Args:
        score_fn: maps a full hyperparameter dict to a scalar score where
            higher is better (e.g. mean cross-val accuracy).
